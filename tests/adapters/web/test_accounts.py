...
class TestRegisterAccount:
    ...
    @pytest.mark.integration
    @pytest.mark.parametrize("json", (True, False))
    async def test_duplicate(self, fake, aiohttp_client, app, passport, json, user):
        app["passport"] = passport
        client = await aiohttp_client(app)
        ...
        account = Account(key=0, name=name, user=user, balance=[Balance(month=month.date())])
        ...
        async with app["db"].acquire() as conn:
            ...
            accounts = [
                Account(
                    ...
                    balance=[
                        Balance(
                            rest=Decimal("-838.00"), expenses=Decimal("838.00"), month=month.date()
                        )
                    ],
                ),
                Account(
...
class TestUpdateAccount:
    ...
    @pytest.mark.integration
    @pytest.mark.parametrize("json", (True, False))
    async def test_success(self, fake, aiohttp_client, app, passport, json, user):
        app["passport"] = passport
        client = await aiohttp_client(app)
        ...
        account = Account(
            ...
            user=user,
            balance=[
                Balance(rest=Decimal("-838.00"), expenses=Decimal("838.00"), month=month.date())
            ],
        )
