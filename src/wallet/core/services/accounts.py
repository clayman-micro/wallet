...
class AccountService(Service[Account, AccountFilters, AccountPayload]):
    async def add(self, payload: AccountPayload, dry_run: bool = False) -> Account:
        ...
        self._logger.info(
            "Add new account", user=account.user.key, name=account.name, dry_run=dry_run,
        )
    ...
    async def remove(self, entity: Account, dry_run: bool = False) -> None:
        ...
        self._logger.info(
            "Remove account", user=entity.user.key, name=entity.name, dry_run=dry_run,
        )

    async def find(self, filters: AccountFilters) -> AsyncGenerator[Account, None]:
