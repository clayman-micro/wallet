...
class AddBulkUseCase(OperationUseCase):
    ...
    async def execute(
        self, payload: BulkOperationsPayload, dry_run: bool = False
    ) -> OperationStream:
        account_stream = self.accounts.find(
            filters=AccountFilters(user=payload.user, keys=payload.account_keys)
        )

        category_stream = self.categories.get_or_create(
            CategoryFilters(
                user=payload.user,
                keys=payload.category_keys,
                names=payload.category_names,
            )
        )
